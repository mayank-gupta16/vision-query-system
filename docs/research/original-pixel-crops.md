...
ASCII destinations below an already-created absolute root, opens every path
component without following symlinks, creates the final file exclusively, and
never includes caller paths or bytes in its errors. The later EvidenceStore
still owns the accepted CAS and commit protocol.
...
Traversal and symlink-parent attempts were rejected, no outside file appeared,
and the written in-root artifact matched its descriptor.
...
and peak-RSS observations are retained in the machine-readable receipt. They
establish a baseline, not a cross-machine performance claim or optimization
target.
