...
worker from the root-owned runtime through systemd and bubblewrap. Before launch,
the complete non-writable runtime tree is checked against a pinned manifest and
content digest. The worker is non-root, has no network namespace, uses
no-new-privileges plus seccomp, and applies Landlock read/execute access only to
the immutable runtime/library mounts. CPU, memory, tasks, wall time, output,
geometry, frame count, and decoded bytes are bounded; cancellation and breaches
kill the whole cgroup and verify that the unit stopped.
...
pixel-hash expectation. Observed per-fixture wall time was 105–139 ms and peak
cgroup memory was 14.0 MB. Corrupt, oversized, traversal, symlink, timeout, and
cancellation cases all returned their expected structured failures. A live
cancellation check observed two process identities in one transient unit and
verified that neither survived whole-cgroup termination. A successful worker
result also requires its non-root, no-new-privileges, no-network, and
