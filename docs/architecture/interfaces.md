...
idempotent commit/discard, bounded integrity inspection/inventory, and
coordinator-authorized deletion. `writer_session()` lets the future coordinator
hold the shared store lock across filesystem and SQLite intent/reference steps;
the filesystem adapter itself never decides whether an artifact is referenced
or orphaned.
