...
No model code, weights, hosted model API, or model runtime is approved for the
product or release. The following short list is approved only for isolated
issue #21 evaluation; no artifact is committed, packaged, or exposed by a
public API.
...
research lock. A benchmark selection changes only the evaluation status. Adding
weights or a runtime to an install, container, service, or release requires a
separate implementation issue and shipping review.
